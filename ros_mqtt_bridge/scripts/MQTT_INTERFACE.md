...
| 3 | `aoa/ai_guide/guide/status` | JSON（`{"running": bool}`） | `mqtt_mssn_bridge` | 2 Hz |
| 4 | `aoa/ai_guide/guide/mode` | JSON（`{"mode": string}`） | `mqtt_mssn_bridge` | 2 Hz |
| 5 | `aoa/ai_guide/guide/tracking` | JSON（`{"tracking": bool}`） | `mqtt_mssn_bridge` | 2 Hz |
...
### 4.3 `aoa/ai_guide/guide/status` — 引导动作状态

JSON：`{"running": true}` = `mqtt_waypoint_bridge` 运行中，`{"running": false}` = 未运行。

### 4.4 `aoa/ai_guide/guide/mode` — 当前拦截模式

JSON：`{"mode": "head-on"}` | `{"mode": "tail"}` | `{"mode": "unknown"}`（未收到 `/uav_guide/intercept_mode` 时）。

### 4.5 `aoa/ai_guide/guide/tracking` — TRACKING 状态

JSON：`{"tracking": true}` = 已进入 TRACKING 模式（读 `/uav_guide/tracking_mode`），`{"tracking": false}` = 未进入。
...
| 状态 `nav/status`、`guide/status` | `{"running": <bool>}` | 布尔 |
| 状态 `guide/mode` | `{"mode": "<head-on\|tail\|unknown>"}` | 字符串 |
| 状态 `guide/tracking` | `{"tracking": <bool>}` | 布尔 |
...
sequenceDiagram
    ...
    M->>B: aoa/ai_guide/guide/status = {"running": true}
    M->>B: aoa/ai_guide/guide/mode = {"mode": "head-on"}
    C->>B: aoa/ai_guide/guide/mode_cmd = {"mode": "tail"}
    B->>M: rostopic 切换模式
    M->>B: aoa/ai_guide/guide/mode = {"mode": "tail"}
    M->>B: aoa/ai_guide/guide/tracking = {"tracking": true}  (进入 TRACKING)
...
| `mqtt.mode_status_topic` | `aoa/ai_guide/guide/mode` | 同上 |
| `mqtt.tracking_status_topic` | `aoa/ai_guide/guide/tracking` | 同上 |
...
| `decodeStateBool` / `encodeStateBool` | `aoa/ai_guide/nav/cmd`、`guide/cmd`、`*status` | `{"start"\|"running"\|"tracking": bool}` |
| `decodeStateString` / `encodeStateString` | `aoa/ai_guide/guide/mode_cmd`、`guide/mode` | `{"mode": string}` |
