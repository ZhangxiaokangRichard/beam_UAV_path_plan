#!/usr/bin/env bash
# 停止 udp_mssn_bridge 与 roscore 的 tmux 会话（供 systemd ExecStop 使用）
set -eo pipefail
...
pkill -f '[u]dp_mssn_bridge.launch' 2>/dev/null || true      # roslaunch
pkill -f '[l]ib/ros_udp_bridge/udp_mssn_bridge' 2>/dev/null || true  # 节点
